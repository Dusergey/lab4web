class Tovar{
	constructor(title, brand, price, capacity, oldPrice){
		this.title = title
		this.brand = brand
		this.price = price
...
allTovars = [];
t1 = new Tovar("Пенка очищающая AHA + KAOLIN GEL CLEAN", "Evasion", 2400, "0.5 мл", 2400);
allTovars.push(t1);
t1 = new Tovar("Мицелярная вода Time Machine Water Clean", "Storyderm", 2520, "0.5 мл", 2772);
allTovars.push(t1);
t1 = new Tovar("Гель для умывания Clinic-a Clean", "Storyderm", 2392, "0.5 мл", 2392);
allTovars.push(t1);
t1 = new Tovar("Пенка 3D & COLLAGEN", "Isov (Sorex)", 2308, "0.5 мл", 2308);
allTovars.push(t1);
t1 = new Tovar("Пенка COMPLEX AC BUBBLE CLEANSER", "Isov (Sorex)", 3032, "0.5 мл", 3032);
allTovars.push(t1);
t1 = new Tovar("Энзимный йогурт SOFT ENZYME EMULSION", "Evasion", 4000, "0.5 мл", 4000);
allTovars.push(t1);
t1 = new Tovar("Гель для умывания SKIN HYDRATION CLEANSING GEL", "Isov (Sorex)", 4320, "0.5 мл", 4320);
allTovars.push(t1);
t1 = new Tovar("Пенка SMART FORM PEELING DEEP CLEANSER", "Isov (Sorex)", 3995, "0.5 мл", 3995);
allTovars.push(t1);
t1 = new Tovar("Товар id-001", "Isov (Sorex)", 3650, "0.5 мл", 3995);
allTovars.push(t1);
t1 = new Tovar("Товар id-002", "Reandne Pro", 4320, "1 мл", 4320);
allTovars.push(t1);
t1 = new Tovar("Товар id-003", "Cellviderm", 3650, "0.5 мл", 3995);
allTovars.push(t1);
t1 = new Tovar("Товар id-004", "Коллост", 5500, "1 мл", 5500);
allTovars.push(t1);
t1 = new Tovar("Товар id-005", "Linerase", 2900, "1 шт.", 2900);
allTovars.push(t1);
t1 = new Tovar("Товар id-006", "Revofil", 2500, "1 шт.", 2500);
allTovars.push(t1);
t1 = new Tovar("Товар id-006", "Reandne Pro", 2200, "1 пара", 2200);
allTovars.push(t1);
...
for(let i=0; i<capacities.length; i++){
	capacities[i].addEventListener('click', ()=>{
		if(selected_caps.indexOf(capacitiesTitle[i].innerHTML) !== -1){ //УДАЛЕНИЕ ОБЪЁМА ИЗ СПИСКА
			...
			capacitiesImg[i].src = "check-box.png";
		}
		fill(allTovars,chosenVar,selected_brands,selected_caps); // Заполнение
	});
}
...
function fill(allTovars, chosenVar, selected_brands, selected_caps){
	...
	selectedItems3.forEach((e)=>{
		contentHTML += `<div class="sell-item">
							...
							<img class="mainImg" src="cream.png">
							<p>${e.title}</p>`;
							if(e.price != e.oPrice){
